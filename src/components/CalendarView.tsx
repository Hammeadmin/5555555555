...
function CalendarView() {
  ...
  const handleFilterByCityAssignments = () => {
    ...
    const usersInCity = users.filter(u => u.cities?.includes(selectedCity)).map(u => u.id);
    const teamsInCity = teams.filter(t => t.cities?.includes(selectedCity)).map(t => t.id);

    setSelectedUsers(usersInCity);
    setSelectedTeams(teamsInCity);
};
 ...
 const handleLeadOutcome = async (lead: Lead | null | undefined, outcome: 'won' | 'lost') => {
  ...
  // Requirement 1: If a user or team is selected, find their assigned cities.
  ...
  const openOrders = orders.filter(order => order.status === 'öppen_order');

  // If a specific city is selected in the filter, it takes top priority.
  if (selectedCity) {
    return openOrders.filter(order => order.customer?.city === selectedCity);
  }

  // If teams/users are selected, filter orders to match their cities.
  if (uniqueRelevantCities.length > 0) {
    return openOrders.filter(order => uniqueRelevantCities.includes(order.customer?.city || ''));
  }
  
  // If no filters are active, return all open orders.
  return openOrders;
 ...
 const [eventForm, setEventForm] = useState<Partial<EventFormData>>({});
  ...
  const initializeCalendar = async () => {
    ...
    try {
      ...
      const [usersResult, teamsResult, userTeamsResult, leadsResult, citiesResult] = await Promise.all([
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 flex items-center space-x-3">
          ...
          <CalendarFilters
            ...
            onClearFilters={handleClearFilters}// Add this new prop
          />
          <button
...
<div className="space-y-2 max-h-60 overflow-y-auto">
  {filteredOrders.map(order => (
    ...
    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-center">
        ...
        >
            Visa teams/arbetare tilldelade till detta område
        </button>
    </div>
)}
        </div>
  ...
  <EventModal
    ...
    availableTeams={availableTeamsForModal}
